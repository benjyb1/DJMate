...
class DatabaseManager:
    ...
    def _initialize_clients(self):
        ...
        if not self.supabase_url or not self.supabase_key:
            logger.warning("⚠️  SUPABASE credentials not found. Database functionality limited.")
            return
    ...
    async def get_tracks_by_semantic_filter(
        ...
        try:
            ...
            query = self.client.table("tracks").select(
                "trackid, filepath, title, artist, album, bpm, key, duration, "
            ...
            if bpm_range and len(bpm_range) == 2:
                query = query.gte("bpm", bmp_range[0]).lte("bmp", bpm_range[1])
