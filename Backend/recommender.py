...
class DJRecommendationEngine:
    ...
    async def get_intelligent_recommendations(
        ...
        # Get candidate tracks based on semantic filters from new schema
        candidates = await self.db.get_tracks_by_semantic_filter(structured_query)
        ...
        for candidate in candidates:
            candidate_labels = await self.db.get_track_labels(candidate["track_id"])
    ...
    async def _compute_comprehensive_score(
        ...
        # 2. Semantic tag overlap from track_labels
        scores["semantic_overlap"] = self._compute_semantic_overlap(
            candidate_labels.get("tags", []),
            query.get("tags", [])
        )

        # 3. Vibe compatibility from track_labels
        scores["vibe_compatibility"] = self._compute_vibe_compatibility(
            candidate_labels.get("vibe_descriptors", []),
            query.get("vibe_descriptors", [])
        )

        # 4. BPM compatibility from track (bpm is in tracks table)
        ...
        # 5. Harmonic compatibility from track (key is in tracks table)
        ...
        # 6. Energy flow compatibility from track_labels
        candidate_energy = candidate_labels.get("energy", 0.5)
        context_energy = context_labels.get("energy", 0.5) if context_labels else 0.5
